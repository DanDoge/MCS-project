...
class FITC_network:
    ...
    def init_hypers_Gaussian(self, x_train, y_train):
        ...
        for d in range(Dout1):
            eta1_R0.append(np.random.randn(int(M1*(M1+1)/2), ))
            eta20.append(np.random.randn(int(M1), ))
        ...
        for i in range(1, self.no_layers):
            ...
            eta2i = []
            for d in range(Douti):
                eta1_Ri.append(np.random.randn(int(Mi*(Mi+1)/2), ) / 10)
                eta2i.append(np.random.randn(int(Mi), ) / 10)
