...
class AEPDGP:
    ...
    def compute_energy(self, params, Xb, yb, N_train, compute_logZ=False):
        ...
        for i in range(no_layers):
            ...
            grad_all['eta1_R'][i] = [np.zeros([int(M*(M+1)/2), ]) for d in range(Dout)]
    ...
    def train(self, X_train, y_train, y_train_origin, X_test, y_test, no_epochs, n_per_mb,
              lrate=0.001, compute_test=False, reinit_hypers=True, compute_logZ=False,
              file_time=None, file_rmse=None, file_llh=None):
        ...
        class PlusOne(object):
            def __init__(self, size, split):
                self.X = np.random.random(size = (size, 1)) - 1.0
                self.Y = np.sin(self.X).reshape(size)
                self.Xtrain = self.X[: int(size * split)]
                self.Xtest = self.X[int(size * split): ]
                self.Ytrain = self.Y[: int(size * split)]
                self.Ytest = self.Y[int(size * split): ]

        class PlusTwo(object):
            def __init__(self, size, split):
                self.X = np.random.random(size = (size, 1))
                self.Y = np.cos(self.X).reshape(size)
                self.Xtrain = self.X[: int(size * split)]
                self.Xtest = self.X[int(size * split): ]
                self.Ytrain = self.Y[: int(size * split)]
                self.Ytest = self.Y[int(size * split): ]

        data1 = PlusOne(1000, 0.8)
        data2 = PlusTwo(1000, 0.8)
        '''
        ...
        try:
            ...
            while (not check):
                ...
                if compute_test:
                    ...
                    my, vy = self.predict(np.append(X_train, X_test, axis=0))

                    my_train = my[: X_train.shape[0]]
                    vy_train = vy[: X_train.shape[0]]

                    my_test = my[X_train.shape[0]: ]
                    vy_test = vy[X_train.shape[0]: ]

                    test_nlli = compute_test_nll(y_test, my_test, vy_test, self.lik, median=False)
                    test_rmsi = compute_test_error(y_test, my_test, self.lik, median=False)
                    train_nlli = compute_test_nll(y_train_origin, my_train, vy_train, self.lik, median=False)
                    train_rmsi = compute_test_error(y_train_origin, my_train, self.lik, median=False)
                    #print "\t logZ: %.5f, test mnll: %.5f, test rms: %.5f" % (energy, test_nlli, test_rmsi)
                    file_rmse.write('%.6f %.6f\n' % (train_rmsi, test_rmsi))
                    file_rmse.flush()
                    os.fsync(file_rmse.fileno())

                    file_llh.write('%.6f %.6f\n' % (train_nlli, test_nlli))
                    file_llh.flush()
                    os.fsync(file_llh.fileno())
    ...
    def init_adam(self, adamlrate=0.001):
        ...
        for i in range(nolayers):
            ...
            v_eta1_Ri = [np.zeros((int(Mi * (Mi+1) / 2), )) for d in range(Douti)]
